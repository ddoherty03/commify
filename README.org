* Commify
 An Emacs package to quickly insert or remove grouping commas (or other
 grouping character) in the number under or adjacent to the cursor, or for all
 numbers within the region, with a single command.  This is an otherwise
...
Simply =package-install commify= with the Emacs package manager.  This code is
up on MELPA.
